...
• Date: {offer_info.get('date', 'N/A')}
    ...
    def _debug_export_datasets_before_merge(self, master_df, comparison_df, offer_info):
        ...
        try:
            ...
            def prepare_dataset_for_export(df):
                ...
                # Create final column list: essential columns first, in correct order
                final_columns = available_essential_columns.copy()
                
                # Add any remaining non-essential columns (but not the unwanted ones)
                remaining_columns = [col for col in export_df.columns if col not in essential_columns]
                final_columns.extend(remaining_columns)
    ...
    def _on_confirm_row_review(self):
        ...
        result = super(type(self), self)._on_confirm_row_review() if hasattr(super(type(self), self), '_on_confirm_row_review') else None
        # If this was a comparison workflow, do the export and stop
        if getattr(self, '_pending_comparison_export', False):
